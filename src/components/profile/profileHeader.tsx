...
export default function ProfileHeader({ owner }: ProfileHeaderProps) {
  ...
  return (
    <div className="bg-[#1B1B22] rounded-[20px] flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 py-6 px-6 border-b border-gray-700 gap-4">
      ...
      {/* Right side: Social Links + Follow Button */}
      <div className="flex items-center space-x-2 sm:space-x-3 flex-shrink-0">
        
        <SocialLink href={owner.linkedInUrl} icon={Linkedin} label={`${owner.name} on LinkedIn`} />
        <SocialLink href={owner.twitterUrl} icon={LinkIcon} label={`${owner.name} on X/Twitter`} />

      
        <FollowButton
              technologyOwnerId={owner.id}
              initialIsFollowing={!!owner.isFollowedByUser}
